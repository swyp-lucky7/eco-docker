...
CREATE TABLE IF NOT EXISTS character_detail (
  ...
  created_at DATETIME(6) DEFAULT NULL,
  updated_at DATETIME(6) DEFAULT NULL,
  PRIMARY KEY (character_image_id),
...
CREATE TABLE IF NOT EXISTS gifticon_user (
  ...
  UNIQUE KEY uk_character_user (character_user_id),
  KEY idx_user (user_id),
  CONSTRAINT fk_character_user FOREIGN KEY (character_user_id) REFERENCES character_user (character_user_id),
  CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
